...
class yourCart extends Component {
  ...
  async componentDidMount() {
    const { cartItems,shoes } = this.state;
    let totalPrice = 0;
  ...
  const updatedItems = shoes.map((item) => ({
    ...item,
    addedToCart: cartItems.some((cartItem) => cartItem.id === item.id) ? true : false,
  }));
  this.setState(({
    shoes: updatedItems
  }))
  ...
  render() {
    const addedShoes = this.state.cartItems;
